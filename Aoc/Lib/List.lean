...
def List.splitAt2 (n : Nat) (l : List α) : List α × List α := go l n #[] where
...
def List.count2 (p : α → Bool) : List α → Nat := (·.filter p |>.length)

-- def List.mapJoin (f : α → List β) (l : List α) : List β := l.map f |> List.flatten
