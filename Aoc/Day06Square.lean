...
-- (does not actually work, since only counts square loops in single pass)
...
def countCrosses (grid: Array (Array Char)) : Nat :=
  grid.foldl (λ count row =>
    count + row.foldl (λ count cell =>
      count + if cell == 'X' then 1 else 0
    ) 0
  ) 0
